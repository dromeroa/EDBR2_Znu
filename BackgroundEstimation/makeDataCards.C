...
void makeDataCards(std::string key, Int_t mass){
  ...
  TFile *sig  = TFile::Open(Form("workSpaces/CMS_ZZnunu_RSG_%s_%d_13TeV.root",  key.c_str(), mass));
  ...
  ZZnunu_13TeV->writeToFile(Form("workSpaces/Workspace2/CMS_ZZnunu_bkgsig_%d_%s_13TeV.root",mass,key.c_str()));

  // create datacard
  const char *outFile = Form("dataCards/datacards2/CMS_ZZnunu_%d_%s_13TeVTEST.txt",mass,key.c_str());
  std::ofstream ofs;
