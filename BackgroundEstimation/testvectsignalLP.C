...
void testvectsignalLP(std::string key){
...
//    ZZ_sig.fitTo(ds, Minimizer("Minuit"), Range(low[mass],upp[mass]), SumW2Error(kTRUE));
    ...
    RooPlot* frame1 = candTMass.frame(Title("#bf{CMS} Preliminary #sqrt{s} = 13 TeV"));
...
//    ZZ_sig.paramOn(frame1,Layout(0.50,0.9,0.6));
    ...
    fPads2 = new TPad("pad2", "", 0.00, 0.06, 0.99, 0.23);
...
//    TLegend *leg1 = new TLegend(0.50,0.6,0.9,0.9);
    TLegend *leg1 = new TLegend(0.1,0.9,0.40,0.7);
