...
void test01abrilLP(std::string key)
{
...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  RooRealVar mu1("mu1", "average", 79.6, 0, 200);
  RooRealVar sigma1("sigma1", "sigma", 100, 0, 200);
  RooRealVar mu2("mu2", "average", 34, 0, 100);
  RooRealVar sigma2("sigma2", "sigma", 100, 0, 200); 
  ...
  RooRealVar lambda("lambda", "exponential slope", 0.1, .0, 1.0);
...
// RooFitResult *rf3 = emodel3.fitTo(bkg2, Save(1), Range(40,160));
// RooFitResult *rf3a = emodel3a.fitTo(bkg2, Save(1),Range(40,160));
  ...
  frame1a->GetXaxis()->SetTitle("Pruned Mass (GeV)");
  ...
  frame1a->SetTitle(""); 
  frame1a->Draw();
  TPaveLabel *t1 = new TPaveLabel(0.7,0.85,0.9,1.0, Form("#chi^{2}/dof = %f", chi2),"brNDC");
  t1->Draw();
  // SAVE THE FILE 
...
//  model3.plotOn(frame3,RooFit::Invisible());
  bkg2.plotOn(frame3,Name("mc3"),Binning(xbins),Range(40,160));
  model3.plotOn(frame3,Name("fitmc3"), LineColor(kRed),Range(40,160));
  model3.plotOn(frame3,Name("error3"),VisualizeError(*rf3,1),FillColor(kAzure-2), FillStyle(3002),Range(40,160));
  model3.paramOn(frame3,Layout(0.50,0.9,0.6));
  ...
  TPad* fPads1b = NULL;
  TPad* fPads2b = NULL;
  fPads1b = new TPad("pad1", "", 0.00, 0.17, 0.99, 0.99);
  fPads2b = new TPad("pad2", "", 0.00, 0.04, 0.99, 0.25);
  ...
  frame3->Draw();
  // The Legend
  TLegend *leg1b = new TLegend(0.50,0.6,0.9,0.9);
  leg1b->SetHeader(legTitle[key].c_str());
  leg1b->AddEntry("mc3",  "Subdominant Background ",    "ep");
  leg1b->AddEntry("fitmc3","Gauss + Gauss fit Function", "l");
  ...
  frame3a->GetXaxis()->SetTitle("Pruned Mass (GeV)");
 ...
 RooAddPdf model_ext("model_ext","sum of extended models",RooArgList(emodel1,emodel4));

 c1.setConstant(true);
 offset1.setConstant(true);
 width1.setConstant(true);
 mu3.setConstant(true);
 sigma3.setConstant(true);
 lambda.setConstant(true);
 funfrac2.setConstant(true);
 ...
 model_ext.plotOn(frame5,Name("subdominantMC"),Components("emodel4"),FillColor(kMagenta-4),VLines(),DrawOption("F"),Range(40,160));
